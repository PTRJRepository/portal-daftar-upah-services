...
describe('payslip print CSS', () => {
  it('uses the full A4 page for a 2x2 payslip grid with a small print-safe page margin', () => {
    ...
    expect(css).toMatch(/\.payslip-a4-page\s*{[^}]*width:\s*210mm;[^}]*min-height:\s*297mm;[^}]*padding:\s*3mm;/);
    expect(css).not.toMatch(/\.payslip-a4-page\s*{[^}]*\n\s*height:\s*297mm;/);
    expect(css).toMatch(/\.payslip-grid\s*{[^}]*width:\s*100%;[^}]*min-height:\s*calc\(297mm - 6mm\);/);
    expect(css).toMatch(/\.payslip-grid\s*{[^}]*grid-template-columns:\s*1fr 1fr;[^}]*grid-template-rows:\s*repeat\(2,\s*auto\);/);
    expect(css).toMatch(/\.payslip-grid\s*{[^}]*gap:\s*0;/);
    expect(css).toMatch(/@media print[\s\S]*\.payslip-grid\s*{[\s\S]*width:\s*100%\s*!important;[\s\S]*min-height:\s*calc\(297mm - 6mm\)\s*!important;/);
    expect(css).toMatch(/@media print[\s\S]*\.payslip-grid\s*{[\s\S]*grid-template-rows:\s*repeat\(2,\s*auto\)\s*!important;/);
  ...
  it('prints dashed cut marks between the four portrait payslips', () => {
    ...
    expect(css).toMatch(/\.payslip-cut-line--half\s*{[\s\S]*top:\s*50%;/);
  ...
  it('centers and lifts the take-home-pay banner so it is prominent but not edge-to-edge', () => {
    ...
    expect(css).toMatch(/\.payslip-card-content\s*{[\s\S]*overflow:\s*visible;/);
  ...
  it('reserves printable space for take-home-pay and keeps overflow visible', () => {
    expect(css).toMatch(/\.payslip-card-content\s*{[\s\S]*flex:\s*1 1 auto;/);
    ...
    expect(css).toMatch(/\.payslip-card\s*{[\s\S]*overflow:\s*visible;/);
    expect(css).toMatch(/\.payslip-card-column\s*{[\s\S]*overflow:\s*visible;/);
  });
  ...
  it('centers the payslip header and keeps the header logo compact', () => {
    ...
    expect(css).toMatch(/\.payslip-card-header\s*{[\s\S]*padding:\s*0\.28mm 0\.5mm 0\.28mm 0\.5mm;/);
    expect(css).toMatch(/\.payslip-card-company\s*{[\s\S]*display:\s*grid;/);
    expect(css).toMatch(/\.payslip-card-company\s*{[\s\S]*grid-template-columns:\s*6\.4mm minmax\(0,\s*auto\) 6\.4mm;/);
    ...
    expect(css).toMatch(/\.payslip-header-logo\s*{[\s\S]*width:\s*6\.4mm;/);
  });

  it('uses a readable scale for both screen preview and print while keeping four slips on one portrait page', () => {
    expect(css).toMatch(/\.payslip-card\s*{[\s\S]*padding:\s*1\.2mm 1\.4mm;/);
    expect(css).toMatch(/\.payslip-card\s*{[\s\S]*font-size:\s*7\.15pt;/);
    expect(css).toMatch(/\.payslip-card\s*{[\s\S]*line-height:\s*1\.18;/);
    expect(css).toMatch(/\.payslip-card-company\s*{[\s\S]*font-size:\s*8\.25pt;/);
    expect(css).toMatch(/\.payslip-card-title\s*{[\s\S]*font-size:\s*7\.75pt;/);
    expect(css).toMatch(/\.payslip-card-period\s*{[\s\S]*font-size:\s*6\.8pt;/);
    expect(css).toMatch(/\.payslip-card-info\s*{[\s\S]*font-size:\s*6\.65pt;/);
    expect(css).toMatch(/\.payslip-activity-summary\s*{[\s\S]*font-size:\s*6\.5pt;/);
    expect(css).toMatch(/\.payslip-column-header\s*{[\s\S]*font-size:\s*7\.15pt;/);
    expect(css).toMatch(/\.payslip-item\s*{[\s\S]*font-size:\s*6\.85pt;/);
    expect(css).toMatch(/\.payslip-item-value\s*{[\s\S]*width:\s*100%;/);
    expect(css).toMatch(/\.payslip-subtotal-line\s*{[\s\S]*font-size:\s*6\.6pt;/);
    expect(css).toMatch(/\.payslip-total-line\s*{[\s\S]*font-size:\s*7\.15pt;/);
    expect(css).toMatch(/@media print[\s\S]*\.payslip-card\s*{[\s\S]*padding:\s*1\.2mm 1\.4mm\s*!important;/);
    expect(css).toMatch(/@media print[\s\S]*\.payslip-card\s*{[\s\S]*font-size:\s*7\.15pt\s*!important;/);
  });
  ...
  it('keeps component labels and amounts close together while preserving aligned amount columns', () => {
    ...
    expect(css).toMatch(/\.payslip-item-label\s*{[\s\S]*white-space:\s*normal;/);
    expect(css).toMatch(/\.payslip-item-label\s*{[\s\S]*overflow-wrap:\s*anywhere;/);
  ...
  it('uses print-stable A4 sizing when html2pdf captures the cloned payslip container', () => {
    ...
    expect(css).toMatch(/\.pdf-export-active\.payslip-print-container\s+\.payslip-a4-page\s*{[\s\S]*width:\s*210mm\s*!important;[\s\S]*min-height:\s*297mm\s*!important;/);
    ...
    expect(css).toMatch(/\.pdf-export-active\.payslip-print-container\s+\.payslip-grid\s*{[\s\S]*grid-template-rows:\s*repeat\(2,\s*auto\)\s*!important;/);
    expect(css).toMatch(/\.pdf-export-active\.payslip-print-container\s+\.payslip-card\s*{[\s\S]*padding:\s*1\.2mm 1\.4mm\s*!important;[\s\S]*font-size:\s*7\.15pt\s*!important;/);
  });
  ...
  it('does not clip or ellipsize payslip text', () => {
    expect(css).not.toContain('text-overflow: ellipsis');
    expect(css).not.toContain('overflow: hidden');
    expect(css).not.toContain('overflow: hidden !important');
    expect(css).toMatch(/\.payslip-info-value\s*{[\s\S]*white-space:\s*normal;/);
    expect(css).toMatch(/\.payslip-info-value\s*{[\s\S]*overflow-wrap:\s*anywhere;/);
  });
});
