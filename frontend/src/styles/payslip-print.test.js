...
describe('payslip print CSS', () => {
  it('uses the full A4 page for a 2x4 payslip grid with a small print-safe page margin', () => {
    ...
    expect(css).toMatch(/@media print[\s\S]*\.payslip-grid\s*{[\s\S]*grid-template-rows:\s*repeat\(4,\s*1fr\)\s*!important;/);
  ...
  it('prints dashed cut marks between the eight portrait payslips', () => {
    ...
    expect(css).toMatch(/\.payslip-cut-line--quarter\s*{[\s\S]*top:\s*calc\(25%/);
    expect(css).toMatch(/\.payslip-cut-line--half\s*{[\s\S]*top:\s*50%;/);
    expect(css).toMatch(/\.payslip-cut-line--three-quarter\s*{[\s\S]*top:\s*calc\(75%/);
  ...
  it('centers the payslip header and keeps the header logo compact', () => {
    ...
    expect(css).toMatch(/\.payslip-card-company\s*{[\s\S]*grid-template-columns:\s*5\.8mm minmax\(0,\s*auto\) 5\.8mm;/);
    ...
    expect(css).toMatch(/\.payslip-header-logo\s*{[\s\S]*width:\s*5\.8mm;/);
  });
