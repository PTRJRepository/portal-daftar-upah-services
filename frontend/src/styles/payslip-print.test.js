...
describe('payslip print CSS', () => {
  it('uses the full A4 page for a 2x3 payslip grid with a small print-safe page margin', () => {
  ...
  it('uses a larger readable scale for both screen preview and print while keeping six slips on one portrait page', () => {
    expect(css).toMatch(/\.payslip-card\s*{[\s\S]*padding:\s*0\.9mm 1\.1mm;/);
    expect(css).toMatch(/\.payslip-card\s*{[\s\S]*font-size:\s*6\.75pt;/);
    expect(css).toMatch(/\.payslip-card\s*{[\s\S]*line-height:\s*1\.12;/);
    expect(css).toMatch(/\.payslip-card-company\s*{[\s\S]*font-size:\s*7\.8pt;/);
    expect(css).toMatch(/\.payslip-card-title\s*{[\s\S]*font-size:\s*7\.35pt;/);
    expect(css).toMatch(/\.payslip-card-period\s*{[\s\S]*font-size:\s*6\.45pt;/);
    expect(css).toMatch(/\.payslip-card-info\s*{[\s\S]*font-size:\s*6\.3pt;/);
    expect(css).toMatch(/\.payslip-activity-summary\s*{[\s\S]*font-size:\s*6\.2pt;/);
    expect(css).toMatch(/\.payslip-column-header\s*{[\s\S]*font-size:\s*6\.9pt;/);
    expect(css).toMatch(/\.payslip-item\s*{[\s\S]*font-size:\s*6\.5pt;/);
    expect(css).toMatch(/\.payslip-item-value\s*{[\s\S]*width:\s*100%;/);
    expect(css).toMatch(/\.payslip-subtotal-line\s*{[\s\S]*font-size:\s*6\.3pt;/);
    expect(css).toMatch(/\.payslip-total-line\s*{[\s\S]*font-size:\s*6\.9pt;/);
    expect(css).toMatch(/@media print[\s\S]*\.payslip-card\s*{[\s\S]*padding:\s*0\.9mm 1\.1mm\s*!important;/);
    expect(css).toMatch(/@media print[\s\S]*\.payslip-card\s*{[\s\S]*font-size:\s*6\.75pt\s*!important;/);
  });
